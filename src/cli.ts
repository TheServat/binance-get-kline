...
import { formatDate, saveKlineCsv, saveKlineJson } from './utils';
...
async function promptUser(): Promise<Partial<PromptResult>> {
  const { pair, interval, startDate, endDate, fileName, fileType } =
    await prompts(questions);
  return { pair, interval, startDate, endDate, fileName, fileType };
}
